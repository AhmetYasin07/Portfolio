...
const Carousel = (props) => {
    ...
    const handleScrollLeft = () => {
        const element = document.getElementById("carousel-items")
        const length = (element.childElementCount)
        const elementWidth = (length * 100) / (window.getComputedStyle(ReactDOM.findDOMNode(element)).getPropertyValue("min-width").replace(/%/g, ''))
    ...
    const handleScrollRight = () => {
        ...
        const length = (element.childElementCount)
        const elementWidth = (length * 100) / (window.getComputedStyle(ReactDOM.findDOMNode(element)).getPropertyValue("min-width").replace(/%/g, ''))
        if ((scrollCarousel - (100 / length)) < -((length-elementWidth) * (100 / length))) {
            setScrollCarousel(0)
            return
        }
        setScrollCarousel(scrollCarousel => scrollCarousel - (100 / length))
    ...
    return (
        <div className="carousel">
            <div id="carousel" className="carousel-wrapper">
                <div onClick={e => handleScrollLeft(e)} className="carousel-arrow left"><FaAngleLeft/></div>
                <div className="carousel-slider">
                    <div ref={carouselItems} style={{transform: `translateX(${scrollCarousel}%)`, minWidth: carouselWidth}} id="carousel-items" className="carousel-items">
                        {props.children}
                    </div>
                </div>
                <div onClick={e => handleScrollRight(e)} id="carousel-auto-button" className="carousel-arrow right"><FaAngleRight/></div>
            </div>
            <div className="dots">
                {(() => {
                    const element = carouselItems?.current
                    const length = (element?.childElementCount)
                    const elementWidth = element ? Math.ceil((length * 100) / (window.getComputedStyle(ReactDOM.findDOMNode(element)).getPropertyValue("min-width").replace(/%/g, ''))) : null
                    const isDotActive = Math.ceil(Math.abs(scrollCarousel) / (100 / length))
                    const dots = [];
                    for (let i = 0; i < length - (elementWidth - 1); i++) {
                        dots.push(<div onClick={e => handleDotClick(e)} id={i} className={isDotActive === i ? "dot active" : "dot"} key={i}/>);
                    }
                    return dots;
                })()}
            </div>
